#!/usr/bin/env python
"""
Extract parameters from caffemodel and save them as a numpy dict.
Use python2
Codes modified from https://github.com/yjxiong/tensorflow-model-zoo.torch
"""
import argparse
...
class CaffeVendor(object):
    ...
    def _layer2dict(self, layer, version):
        ...
        print(layer_name)
        if len(blobs) > 0:
            blob_dict['{}.weight'.format(layer_name)] = blobs[0] # torch.from_numpy(blobs[0])
            if len(blobs) > 1: # with bias
                blob_dict['{}.bias'.format(layer_name)] = blobs[1] # torch.from_numpy(blobs[1])
            if op == 'SyncBN': # SyncBN or BN
                print('SyncBN')
                blob_dict['{}.running_mean'.format(layer_name)] = blobs[2] # torch.from_numpy(blobs[2])
                blob_dict['{}.running_var'.format(layer_name)] = blobs[3] # torch.from_numpy(blobs[3])
    ...
    def dump_weights(self, filename):
        ...
        np.save(args.dump_weights, self._weight_array_dict)
